...
import { ViewPageNation, Page, ViewSection, PrevButton, NextButton } from "./style/RecordList";
...
export default function View() {
   ...
   if (data?.totalCount && data?.limit) totalPage = Math.ceil(data?.totalCount / data?.limit);
   ...
   return (
      <ViewLayout>
         <ViewSection view={isMobile}>
            ...
            {data?.totalCount ? <RecrodList data={data} page={page} /> : <></>}
            {data?.totalCount ? (
               <ViewPageNation page={PageArr.length + 1}>
                  <PrevButton onClick={PrevHandler} child={page}>
                     Prev
                  </PrevButton>
                  {PageArr[positionPage]?.map((item: number, index: number) => (
                     <Page
                        child={page % limit === 0 ? limit : page % limit}
                        key={index}
                        onClick={() => {
                  ...
                  <NextButton onClick={NextHandler} child={page === totalPage ? page : 0}>
                     Next
                  </NextButton>
               </ViewPageNation>
            ) : (
