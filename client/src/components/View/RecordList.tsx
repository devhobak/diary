...
import { ViewUl, ViewImg, ViewLi, ViewDate, ViewTitle, ViewContent, MoreButton } from "./style/RecordList";
      ...
      <ViewUl view={isMobile}>
         {props.data.logList.map((item, index) => (
            <ViewLi key={index} clickIndex={moreButton} isMobile={isMobile}>
               ...
               {item.content_image ? <ViewImg src={item.content_image} view={isMobile} alt="게시한 이미지" /> : <></>}
               <ViewContent className="content">{item.content_main}</ViewContent>
               <MoreButton isLongContent={longContent[index] as boolean} onClick={() => MoreButtonHandler(index)}>
                  더보기
               </MoreButton>
            </ViewLi>
         ))}
