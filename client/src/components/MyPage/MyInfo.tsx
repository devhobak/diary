...
export default function MyInfo() {
   ...
   const unregister = useMutation(["unregister"], (variable: number) => Unregister(variable, "/api/user/delete"), {
      onSuccess() {
      ...
      onError() {
         toast.error("오류가 발생하였습니다. 관리자에게 문의 부탁드립니다.");
      },
   });
      ...
      <ViewLayout>
         <RecordInfoSection isMobile={isMobile}>
            ...
            <StatisticsNoticeArticle>
               ...
               <TotalBox>
                  <TotalNumberContents>작성한 일기 수(전체)</TotalNumberContents>
                  <TotalNumber>{totalNumber}</TotalNumber>
                  <span>개</span>
               </TotalBox>
               <ButtonBox>
                  <img src={prevButton} alt="이전년도 버튼" onClick={DownClick} />

                  <YearSpan>{currentYear}년</YearSpan>
                  <img src={nextButton} alt="다음년도 버튼" onClick={UpClick} />
               </ButtonBox>
               <Chart />
            </StatisticsNoticeArticle>
            <UserregisterBox>
               ...
               <UnregisterButton onClick={unregisterHandler}>회원탈퇴</UnregisterButton>
            </UserregisterBox>
         </RecordInfoSection>
      </ViewLayout>
   );
