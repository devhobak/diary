...
import { curDateState, selectDateState } from "../../../recoil/atoms/calendarState";
import { modalState } from "../../../recoil/atoms/modalState";
import { ColorState } from "../../../recoil/atoms/recordState";

import { ModalBackground, ModalSection, CloseButton, Date, EditButton } from "../../common/Modal/modal";
...
export default function Record(props: PropType): JSX.Element {
   ...
   //선택한 날짜의 데이터를 저장함.
   const todayRecord = props.data?.filter(item => item.datetime.split(" ")[0] === selectDay).length;

   if (selectDay === fullDate) {
      return (
         <ModalBackground isClose={modal}>
            <ModalSection isClose={modal} color={todayRecord ? `#${diaryArr[0].color}` : color} view={isMobile}>
               ...
               {todayRecord ? (
                  <>
                     ...
                     {edit ? <Edit data={diaryArr} /> : <Diary data={diaryArr} type="today" />}
                  </>
               ) : (
                  <InputSection />
               )}
      ...
      return (
         <ModalBackground isClose={modal}>
            <ModalSection isClose={modal} color={`#${diaryArr[0].color}`} view={isMobile}>
               <h2 className="ir">일상기록</h2>
               <Date>{selectDay}</Date>
               <CloseButton
                  src={closeImg}
